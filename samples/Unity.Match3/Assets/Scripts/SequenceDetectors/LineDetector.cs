...
{
    /// 获取序列（横向 纵向）
    public abstract ItemSequence GetSequence(GameBoard gameBoard, GridPosition gridPosition);

    /// 按方向获取序列
    protected ItemSequence GetSequenceByDirection(GameBoard gameBoard, GridPosition gridPosition, IEnumerable<GridPosition> directions)
    {
        // 被消除的格子集合
        ...
        // 少于3个不能消
        if (solvedSlots.Count < 2) return null;
