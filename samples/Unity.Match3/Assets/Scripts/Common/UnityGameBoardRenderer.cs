...
public class UnityGameBoardRenderer : MonoBehaviour
{
    ...
    /// 获取棋盘格子信息
    public UnityGridSlot[,] GetGridSlots(int level)
    {
        return _gridSlots;
    }
    ...
    private static TileType GetNextAvailableType(TileType type)
    {
        ...
        if (index < groupValues.Length) resultGroup = groupValues[index];
    ...
    private void SetTile(int rowIndex, int columnIndex, TileType type)
    {
        ...
        if (currentTile != null) _tilesPool.RecycleTile(currentTile);
    ...
    private void DisposeGridTiles()
    {
        ...
        foreach (var gridSlotTile in _gridTiles) gridSlotTile.Dispose();
