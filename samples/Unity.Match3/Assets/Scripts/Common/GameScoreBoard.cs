...
namespace Match3
{
    ...
    {
        /// 当序列被消除时
        public void OnSequencesSolved(SolvedData solvedData)
        {
            foreach (var sequence in solvedData.SolvedSequences) RegisterSequenceScore(sequence);
        }

        private static void RegisterSequenceScore(ItemSequence sequence)
        {
            // todo：combo 计分 出兵 
            Debug.Log(GetSequenceDescription(sequence));
        }

        private static string GetSequenceDescription(ItemSequence sequence)
        {
